...
async def subscribe(args, request):
    global ws
    non_auth_method = ["Authenticate",
                       "EstimateMarketOrder",
                       "WatchOrders",
                       "WatchTicker"]
    auth = args.auth
    async with websockets.connect(args.url) as ws:
        ...
        try:
            ...
            if 'tag' in msg and msg['tag']==args.tag:
                ...
                auth = True
            if (request['method'] not in non_auth_method) and (auth == False):
                raise Exception("Needs authentication")
            else:
...
# for other requests, please refer to https://github.com/coinflex-exchange/API/blob/master/WEBSOCKET-README.md
request = {
    # "method": "WatchOrders",
    # "base": Assets['XBTJAN']['id'],
    # "counter": Assets['USDTJAN']['id'],
    # "watch": True,
    "method": "GetBalances",
}

asyncio.get_event_loop().run_until_complete(subscribe(args, request))
